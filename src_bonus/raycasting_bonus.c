...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/06 15:47:01 by sslowpok          #+#    #+#             */
/*   Updated: 2022/10/07 13:29:07 by sslowpok         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_dtype	ray_intersect_x(t_game *game, t_dtype step)
{
	...
	while ((unsigned)check.y < game->map_height && \
			(unsigned)check.x < game->map_length && \
			game->map_array[(unsigned)check.y][(unsigned)check.x] != '1')
...
static void	ray_intersect(t_game *game, double angle, unsigned int ray)
{
	...
	const t_dtype	distance = {game->p_vector_x * (x1.x - game->p_pos_x) + \
		game->p_vector_y * (x1.y - game->p_pos_y),
		game->p_vector_x * (y1.x - game->p_pos_x) + \
		game->p_vector_y * (y1.y - game->p_pos_y)};
...
void	raycasting(t_game *game)
{
	...
	{
		angle = game->p_angle + atanf((float) ray / game->col_center - 1);
		if (angle < 0)
			angle += PI2;
		else if (angle > PI2)
			angle -= PI2;
